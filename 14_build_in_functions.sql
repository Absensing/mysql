...
select
	elt(2, '사과', '딸기', '바나나'),
    field('딸기', '사과', '딸기', '바나나'),
	field_in_set('바나나', '사과,딸기,바나나'),
    instr('사과딸기바나나', '딸기'),
    locate('딸기','사과딸기바나나');
    
    -- format(숫자, 소수점 자리수)
    -- 1000 단위마다 콤마(,) 표시를 해주며 소수점 아래 자릿수 까지 반올림한다.
    select format(123123121231.123123123123123, 3);
    
    -- insert(기준 문자열, 위치, 길이, 삽입할 문자열)
    select insert('내 이름은 아무개입니다.', 7, 3, '조평훈');
    ...
    -- 시작 위치부터 길이 만큼의 문자를 반환(길이를 생각하면 시작 위치부터 끝까지 반환)
    select substring('안녕하세요 반갑습니다.', 7, 2), substring('안녕하세요 반갑습니다.', 7);
    
    -- substring_index(문자열, 구분자, 횟수)
    -- 구분자가 왼쪽부터 횟수 번째 나오면 그 이후의 으른쪽은 버린다.
    ...
    select curdate(), surtime(), now(), sysdate();
    ...
    -- datediff(날짜 1, 낧짜 2)
    -- timediff(날짜 1, 시간 1, 날짜 2 또는 시간 2)
    ...
    -- dayofear : 해당 년도에서 몇 일이 흘렀는지alter
